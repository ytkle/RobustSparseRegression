function [beta resid edf lamin]= RobRidge(X,y,numlam, cualcv,showhist,nkeep)                 
...
msemin=inf; historia=[];
if cualcv==0         %Approximate leave-one-out CV
    imes=2;   %Evaluate prediction error through tau-scale
    for klam=1:nlam    %Try each candidate lambda
        lam=lamdas(klam); deltaesc=deltas(klam);
        %Compute RR-MM
        [betaSE betaMM residSE residMM edfSE edfMM mseMM]=RidSEMM(Xnor,ynor,lam,deltaesc,nkeep);
        mse=mseMM(imes);
        if mse<msemin     %Choose best lammda
            beta=betaMM; msemin=mse; lamin=lam; resid=residMM; edf=edfMM;
        end
        historia=[historia; [lam mse edfMM]];
    end
else   %Actual CV, or test sample
  ...
  for klam=1:nlam
    ...
    [betaSE beta residSE resid edfSE edf]=RidSEMM(Xnor,ynor,lamin,delmin,nkeep);
end
